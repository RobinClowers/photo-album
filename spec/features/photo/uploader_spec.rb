...
describe Uploader do
  let(:path) { "spec/fixtures/files/upload_photos/" }
  let(:full_path) { File.expand_path(path) }
  let(:filename) { "P1120375.suffix.JPG" }
  let(:scrubbed_filename) { "P1120375-suffix.JPG" }
  let(:other_filename) { "P1080205.JPG" }
  let(:all_filenames) { [scrubbed_filename, other_filename] }
  ...
  describe "#upload" do
    it "uploads the file" do
      ...
      expect(album_photos).to have_received(:create)
        .with(scrubbed_filename, File.join(full_path, scrubbed_filename), size, overwrite: false)
    end

    it "passes down the overwrite flag" do
      uploader.upload(path, filename, size, overwrite: true)
      expect(album_photos).to have_received(:create)
        .with(scrubbed_filename, File.join(full_path, scrubbed_filename), size, overwrite: true)
    end
  end
